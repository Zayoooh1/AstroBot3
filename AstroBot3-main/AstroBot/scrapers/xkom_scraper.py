...
def _parse_price_to_cents(price_text: str) -> int | None:
    ...
    try:
        ...
        logging.error(f"Nie udało się sparsować ceny '{price_text}' na grosze: {e}")
        return None
...
def scrape_xkom_product(url: str) -> dict | None:
    ...
    """
    # WAŻNE: Zastąp tą listę działającymi proxy!
    proxies = [
    ...
    if not proxies:
        logging.warning("Brak skonfigurowanych proxy. Scrapowanie może być blokowane.")
    ...
    try:
        ...
        if product_data["name"]:
            ...
            logging.warning(f"Scraping X-Kom: Nie udało się znaleźć kluczowych danych (nazwy) dla {url}")
            return None

    except requests.exceptions.RequestException as e:
        logging.error(f"Błąd żądania HTTP dla {url}: {e}")
        return None
    except Exception as e:
        logging.exception(f"Nieoczekiwany błąd podczas scrapowania {url}: {e}")
        return None
    finally:
