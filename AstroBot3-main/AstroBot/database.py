...
def init_db():
    ...
    cursor.execute("DROP TABLE IF EXISTS watched_products")
    cursor.execute("""
...
# --- Funkcje dla Raportów ---
def get_daily_report_data(guild_id: int):
    """
    Pobiera dane do raportu dziennego.
    """
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    # Dodaj zapytania do bazy danych,
    # aby pobrać dane potrzebne do raportu.
    conn.close()
    return None

def get_weekly_report_data(guild_id: int):
    """
    Pobiera dane do raportu tygodniowego.
    """
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    # Dodaj zapytania do bazy danych,
...
def get_monthly_report_data(guild_id: int):
    """
...
def reactivate_watched_product(product_id: int, user_id: int, guild_id: int, notification_channel_id: int | None = None):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("""
        UPDATE watched_products
        SET is_active = TRUE, user_id_who_added = ?, guild_id = ?, notification_channel_id = ?
        WHERE id = ?
    """, (user_id, guild_id, notification_channel_id, product_id))
    conn.commit()
    conn.close()
...
def get_product_changes_for_report(guild_id: int, hours_ago: int) -> list[dict]:
    ...
    cursor.execute("""
        SELECT 
            ...
            ph.price_cents,
            ph.availability_str,
            wp.last_known_price_cents,
            wp.last_known_availability_str,
            (wp.last_known_price_cents - ph.price_cents) AS price_change
        FROM watched_products wp
        JOIN price_history ph ON wp.id = ph.watched_product_id
        WHERE wp.guild_id = ? AND ph.scan_date >= ?
        ORDER BY wp.product_name
    """, (guild_id, cutoff_timestamp))
    rows = cursor.fetchall()
    conn.close()
    return [dict(row) for row in rows]

def get_top_price_drops(guild_id: int, hours_ago: int, limit: int) -> list[dict]:
    conn = sqlite3.connect(DB_NAME)
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    cutoff_timestamp = int(time.time()) - (hours_ago * 3600)
    cursor.execute("""
        SELECT 
            wp.product_name,
            wp.product_url,
            (ph.price_cents - wp.last_known_price_cents) AS price_drop
        FROM watched_products wp
        JOIN price_history ph ON wp.id = ph.watched_product_id
        WHERE wp.guild_id = ? AND ph.scan_date >= ? AND ph.price_cents IS NOT NULL AND wp.last_known_price_cents IS NOT NULL
        ORDER BY price_drop ASC
        LIMIT ?
    """, (guild_id, cutoff_timestamp, limit))
    rows = cursor.fetchall()
    conn.close()
    return [dict(row) for row in rows]
